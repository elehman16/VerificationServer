...
class XMLReader(Reader):
    ...
    def _get_title(self, article_meta):
        ...
        title = ET.tostring(title_xml, encoding='utf8',
    ...
    def _get_sections(self, body):
        ...
        for i in range(len(children)):
            ...
            if (child.tag == 'sec'):
                ...
                title = ET.tostring(child, method='text',
        ...
        try:
            ...
            if (temp is None):
                ...
                for part in abstract_sections:
                    abstract.append(
                        [part[0], ET.tostring(part[1]).decode('utf-8')])
        except:
            lop = article_meta.find('abstract').findall('p')
            abstract = reduce((lambda x, y: ''.join(
                [x, ET.tostring(y).decode('utf-8')])), lop, "")
            if abstract == '':
    ...
    def _fix_text_(self, text):
        ...
        for t in text:
            if type(t) == str:
                ...
                parsed_text = self._fix_text_(t[0])
                name = 'No title'
    ...
    def _init_article_(self, next_file, article_meta, body):
        ...
        art.get_extra()['reasoning'] = [
            prompt_info['reason'], annotator_info['reason']]
        art.get_extra()['answer'] = [prompt_info['answer'],
                                     annotator_info['answer']]
        art.get_extra()['offsets'] = prompt_info['offset'] + \
            annotator_info['offset']
    ...
    def get_next_article(self, user, next_file=None):
        ...
        path_to_file = self.path + '/PMC' + \
            str(pmc) + '.nxml'  # the path to XML files
